...
func updateConfigFromFile(config *rlogConfig) {
	...
	for scanner.Scan() {
		...
		if len(tokens) != 2 {
			//rlogIssue("Malformed line in config file %s:%d. Ignored.", settingConfFile, i)
			continue
		}
		...
		default:
			//rlogIssue("Unknown or illegal setting name in config file %s:%d. Ignored.", settingConfFile, i)
		}
	}
}
