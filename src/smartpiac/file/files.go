...
func WriteSharedFile(c *config.SmartPiConfig, values *models.ADE7878Readout, balancedvalue float64) {
	var f *os.File
	var err error
	...
	log.Info(logLine)
	sharedFile := filepath.Join(c.SharedDir, "smartpi_values")
...
func UpdateCounterFile(c *config.SmartPiConfig, f string, v float64) float64 {
	t := time.Now()
	var counter float64
...
func ReadCounterFile(c *config.SmartPiConfig, f string) float64 {
	t := time.Now()
	var counter float64
	counterFile, err := os.ReadFile(f)
	if err == nil {
